...
export function CreateKbModal({ isOpen, onClose, onSuccess }: { isOpen: boolean, onClose: () => void, onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg('');
      // 后端 API: 创建知识库
      const res = await http.post('/api/kb/create', {
        title,
        description,
        coverUrl
      });
      ...
      if (res.data.code === 200 || res.data.message === 'success') {
        ...
        setErrorMsg(res.data.message || '创建失败');
      }
    } catch (error: any) {
      setErrorMsg(error.response?.data?.message || '创建失败，请重试');
    } finally {
      ...
      <div className="bg-white dark:bg-[#161b22] rounded-2xl w-full max-w-md overflow-hidden shadow-2xl ring-1 ring-zinc-200 dark:ring-zinc-800 scale-in-center">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-5">
          ...
          <div className="pt-2">
            ...
            >
              {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : '确认创建'}
            </button>
          </div>
        </form>
      </div>
